...
const elanSayisi = document.querySelector(".elan-sayisi");
const carContainer = document.querySelector(".car-container");
let kod = "";
let likedCars = [];
function renderCars(arr) {
  carContainer.innerHTML = "";
  elanSayisi.innerHTML = arr.length + " elan";
  arr.forEach((car) => {
    const kreditIsaresi =
      car.credit == true
    ...
    kod = `<div onclick='showDetails(${
      ...
      <div class="card-body px-0 ">
        <h5 class="card-title fw-bold mb-0 "> ${
          car.price.toLocaleString("en").replace(/,/g, " ") + " " + car.currency
        }</h5>
      ...
      <i class="fa-regular fa-heart" onclick="event.stopPropagation(); addToLikedCars(${
        car.id
      } , this)" style='position:absolute; right:10px ;cursor:pointer; top: 10px;  font-size: 1.5em;  color:white;'></i>
      ${kreditIsaresi}
      ${barterIsaresi}
    `;
...
let currentContainer = databaseCars;
...
function inputBlur() {
  ...
  dropdownInput.classList.toggle("d-none");
...
function inputBlur3() {
  ...
  input3.style.border = "1px solid #dfe3e9";
...
function inputBlur5() {
  ...
  input5.style.border = "1px solid #dfe3e9";
  // input_title3.classList.add("titleVisible");
}
...
function inputBlur6() {
  ...
  input6.style.border = "1px solid #dfe3e9";
}
...
//       document.getElementById(i.split(" ").join("")).classList.remove("d-none");
//     } else {
//       document.getElementById(i.split(" ").join("")).classList.add("d-none");
//     }
//   }
//   console.log(filter);
// }
...
function updateFileLocation() {
  if (window.innerWidth < 991) {
    window.location.href = "index-m.htm";
  } else {
    return;
  }
}
...
sortBy.addEventListener("change", (e) => {
  if (e.target.value == "baha") {
    currentContainer.sort((a, b) => b.price - a.price);
  } else if (e.target.value == "ucuz") {
    currentContainer.sort((a, b) => a.price - b.price);
  } else if (e.target.value == "az") {
    currentContainer.sort((a, b) => a.brand.localeCompare(b.brand));
  } else {
    currentContainer.sort((a, b) => b.brand.localeCompare(a.brand));
  }
  renderCars(currentContainer);
});
